...
public var debugTransport = false
...
internal func httpTransfer(_ surl: String, _ method:String, _ payload:Data?, _ headers: [String], cb:@escaping (_ data:TransportResult)->Void) {
    ...
    if debugTransport, let pl = payload {
      print("Request to " , url," payload :"+String(decoding: pl, as: UTF8.self), to:&standardError)
    }
    ...
    URLSession.shared.dataTask(with: request,completionHandler: { data, response, error in
        ...
        if debugTransport {
            print("Response : " , String(decoding: d, as: UTF8.self), to:&standardError)
        }
...
internal class In3Request {
    ...
    func reportError(req:UnsafeMutablePointer<in3_req_t>) {
        if let error = req.pointee.error {
            ...
            DispatchQueue.main.async {
                self.cb(RequestResult.error(err))
            }
            self.free()
        }
