...
public class In3 {
  ...
  public init(_ config: In3Config) throws {
    var chainId : UInt32 = 1
    if let chainName = config.chainId {
        if chainName == "btc" {
            chainId = UInt32(CHAIN_ID_BTC)
        }
        else if (chainName == "mainnet") {
            chainId = UInt32(CHAIN_ID_MAINNET)
        }
        else if (chainName == "goerli") {
            chainId = UInt32(CHAIN_ID_GOERLI)
        }
        else if (chainName == "ipfs") {
            chainId = UInt32(CHAIN_ID_IPFS)
        }
    }
    transport = httpTransfer
    in3 = in3_for_chain_auto_init(chainId)
