...
public class Contract {
    ...
    public init(in3:In3, abi:[ABI], at:String?=nil) throws {
        ...
        self.abi = abi
        self.hashes = [:]
        self.hashes = try blackMagic(abi: abi, hashes: [:])
    ...
    public init(in3:In3, abiJSON:String, at:String?=nil) throws {
        self.in3=in3
        self.address = at
        self.abi = try JSONDecoder().decode([ABI].self, from: abiJSON.data(using: .utf8)!)
        self.hashes = [:]
        self.hashes = try blackMagic(abi: abi, hashes: [:])
    }
    
    private func blackMagic(abi:[ABI], hashes:[String:ABI]) throws -> [String:ABI]{
        let utils = Utils(in3)
        var mutatedHashes = hashes
        for var a in abi {
            let hash = try utils.keccak(data: a.inputSignature.replacingOccurrences(of: "indexed ", with: ""))
            mutatedHashes[hash] = a
            a.hash = hash
        }
        return mutatedHashes
    }
